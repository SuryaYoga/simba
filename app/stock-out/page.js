...
import {
  ...
  ArrowUpCircle, Package, Calendar, TrendingDown, MoreVertical,
  Pencil, Trash2, Eye, RotateCcw, AlertTriangle, ChevronDown, Check
} from 'lucide-react'
...
const reasonBadge = {
  'Terjual': 'bg-emerald-100 text-emerald-700',
  'Rusak': 'bg-orange-100 text-orange-700',
  'Kadaluarsa': 'bg-rose-100 text-rose-700',
  'Retur': 'bg-blue-100 text-blue-700',
  'Hilang': 'bg-purple-100 text-purple-700',
  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-gray-900 rounded-2xl p-6 w-full max-w-sm">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 text-center mb-3">Transaksi ini akan dihapus dan stok akan dikembalikan otomatis.</p>
        {item && (
          <div className="bg-gray-50 dark:bg-gray-800 rounded-xl p-3 mb-4 text-sm">
            <p className="font-medium text-gray-800 dark:text-gray-100">{item.products?.name}</p>
            <p className="text-gray-500 dark:text-gray-400">{item.date} · -{item.quantity} {item.products?.unit}</p>
          </div>
        )}
...
function ReasonBadge({ reason }) {
  if (!reason) return <span className="text-gray-300 text-xs">-</span>
  return <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${reasonBadge[reason] || 'bg-gray-100 text-gray-600'}`}>{reason}</span>
}

const inputCls = (hasError) =>
  `border rounded-xl p-2.5 text-sm w-full bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-rose-400 transition-colors ${
...
function CustomSelect({ value, onChange, options, placeholder }) {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      <div className={`absolute left-0 top-11 min-w-max bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-xl z-50 overflow-hidden transition-all duration-200 origin-top ${open ? 'opacity-100 scale-100 translate-y-0' : 'opacity-0 scale-95 -translate-y-2 pointer-events-none'}`}>
        {options.map(({ value: val, label }) => (
          <button type="button" key={val} onClick={() => { onChange(val); setOpen(false) }}
            className={`flex items-center gap-3 w-full px-4 py-2.5 text-sm transition-colors whitespace-nowrap ${value === val ? 'bg-blue-50 dark:bg-blue-950 text-blue-600 dark:text-blue-400 font-medium' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700'}`}>
            {label}
...
function SearchableSelect({ value, onChange, options, placeholder, hasError }) {
  ...
  return (
    <div className="relative flex-1" ref={ref}>
      <button type="button" onClick={() => setOpen(o => !o)}
        className={`flex items-center justify-between w-full rounded-xl p-2.5 text-sm border bg-white dark:bg-gray-800 text-left transition-colors ${value ? 'text-gray-900 dark:text-gray-100' : 'text-gray-400 dark:text-gray-500'} ${hasError ? 'border-rose-400 dark:border-rose-500' : open ? 'border-rose-400 ring-2 ring-rose-400' : 'border-gray-200 dark:border-gray-700'}`}>
        <span className="truncate">{current?.label || placeholder}</span>
        <ChevronDown size={14} className={`text-gray-400 shrink-0 ml-2 transition-transform duration-200 ${open ? 'rotate-180' : ''}`} />
      </button>
      <div className={`absolute left-0 top-12 w-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-xl z-50 overflow-hidden transition-all duration-200 origin-top ${open ? 'opacity-100 scale-100 translate-y-0' : 'opacity-0 scale-95 -translate-y-2 pointer-events-none'}`}>
        <div className="p-2 border-b border-gray-100 dark:border-gray-700">
          <div className="relative">
            <Search size={13} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-400" />
            <input ref={inputRef} type="text" value={query} onChange={e => setQuery(e.target.value)} placeholder="Cari produk..."
        ...
        <div className="max-h-52 overflow-y-auto">
          {filtered.length === 0
            ...
            : filtered.map(({ value: val, label }) => (
              <button key={val} type="button" onClick={() => { onChange(val); setOpen(false); setQuery('') }}
                className={`flex items-center gap-3 w-full px-4 py-2.5 text-sm transition-colors ${value === val ? 'bg-blue-50 dark:bg-blue-950 text-blue-600 dark:text-blue-400 font-medium' : 'text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700'}`}>
                {label}
...
export default function StockOutPage() {
  ...
  const [form, setForm] = useState({ product_id: '', quantity: '', old_quantity: 0, reason: '', reason_note: '', note: '', date: new Date().toISOString().split('T')[0] })
  ...
  useEffect(() => {
    ...
    let result = [...history]
    if (search) result = result.filter(h => h.products?.name?.toLowerCase().includes(search.toLowerCase()) || h.products?.barcode?.includes(search))
    if (filterCategory !== 'all') result = result.filter(h => h.products?.category === filterCategory)
    if (filterReason !== 'all') result = result.filter(h => h.reason === filterReason)
    ...
    setFiltered(result)
    setPage(1)
  }, [history, search, filterCategory, filterReason, filterPreset, filterDateFrom, filterDateTo, sortBy])

  useEffect(() => {
    const product = products.find(p => p.id === form.product_id)
    if (!product || !form.quantity) { setStockWarning(''); return }
    const qty = parseInt(form.quantity)
    const available = editId ? product.stock + form.old_quantity : product.stock
    if (qty > available) setStockWarning(`Stok tidak cukup! Tersedia: ${available} ${product.unit}`)
    else if (qty <= 0) setStockWarning('Jumlah harus lebih dari 0')
    else setStockWarning('')
  }, [form.product_id, form.quantity, products, editId, form.old_quantity])

  async function fetchProducts() {
    const { data } = await supabase.from('products').select('id, name, barcode, price, stock, unit, image_url, category').order('name')
    setProducts(data || [])
  }

  async function fetchHistory() {
    setFetching(true)
    const { data } = await supabase.from('stock_out').select('*, products(name, unit, barcode, image_url, category)').order('created_at', { ascending: false })
    setHistory(data || [])
    setFetching(false)
  }
  ...
  async function handleSubmit(e) {
    ...
    if (editId) {
      ...
      const { error } = await supabase.from('stock_out').update({
        product_id: form.product_id, quantity: parseInt(form.quantity),
        reason: form.reason || null, reason_note: form.reason === 'Lainnya' ? form.reason_note : null,
        note: form.note || null, date: form.date, price_at_time: product.price,
      }).eq('id', editId)
      if (error) { setToast({ type: 'error', text: 'Gagal update transaksi' }) }
      else {
        if (difference > 0) await supabase.rpc('decrement_stock', { product_id: form.product_id, amount: difference })
        else if (difference < 0) await supabase.rpc('increment_stock', { product_id: form.product_id, amount: Math.abs(difference) })
        addNotification({ type: 'stock_out', message: `Barang keluar diupdate: ${product.name} -${form.quantity} ${product.unit || ''}`, link: '/stock-out' })
      ...
      const { error } = await supabase.from('stock_out').insert({
        product_id: form.product_id, quantity: parseInt(form.quantity),
        reason: form.reason || null, reason_note: form.reason === 'Lainnya' ? form.reason_note : null,
        note: form.note || null, date: form.date, price_at_time: product.price, user_id: user.id,
      })
      if (error) { setToast({ type: 'error', text: 'Gagal menyimpan data' }); setLoading(false); return }
      await supabase.rpc('decrement_stock', { product_id: form.product_id, amount: parseInt(form.quantity) })
      addNotification({ type: 'stock_out', message: `Barang keluar: ${product.name} -${form.quantity} ${product.unit || ''}`, link: '/stock-out' })
      setToast({ type: 'success', text: 'Barang keluar berhasil dicatat' })
    }
  ...
  async function handleDelete() {
    await supabase.rpc('increment_stock', { product_id: deleteTarget.product_id, amount: deleteTarget.quantity })
    const { error } = await supabase.from('stock_out').delete().eq('id', deleteTarget.id)
    if (error) setToast({ type: 'error', text: 'Gagal hapus transaksi' })
    else setToast({ type: 'success', text: 'Transaksi dihapus & stok dikembalikan' })
  ...
  function handleEdit(item) {
    setEditId(item.id)
    setForm({ product_id: item.product_id, quantity: item.quantity, old_quantity: item.quantity, reason: item.reason || '', reason_note: item.reason_note || '', note: item.note || '', date: item.date })
    setFieldErrors({})
    setShowForm(true)
  }

  function resetForm() {
    setForm({ product_id: '', quantity: '', old_quantity: 0, reason: '', reason_note: '', note: '', date: new Date().toISOString().split('T')[0] })
    setFieldErrors({})
    setEditId(null)
    setStockWarning('')
  }
  ...
  function resetFilter() {
    setSearch(''); setFilterCategory('all'); setFilterReason('all')
  ...
  function handleBarcodeDetected(code) {
    if (scanContext === 'search') {
      setSearch(code); setToast({ type: 'success', text: `Barcode: ${code}` })
    } else {
      const product = products.find(p => p.barcode === code)
      if (product) { setField('product_id', product.id); setToast({ type: 'success', text: `Produk: ${product.name}` }); setShowForm(true) }
      else setToast({ type: 'error', text: 'Produk tidak ditemukan' })
    }
  }
  ...
  const reasonFormOptions = REASONS.map(r => ({ value: r, label: r }))
  ...
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-gray-950">
      ...
      {showScanner && (
        <BarcodeScanner onDetected={code => { handleBarcodeDetected(code); setShowScanner(false) }} onClose={() => setShowScanner(false)} />
      )}
        ...
        {showDetail && (
          <div className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-rose-50 dark:bg-rose-950/40 rounded-xl p-3">
                <p className="text-xs text-gray-400 mb-1">Jumlah Keluar</p>
                <p className="font-bold text-rose-600 dark:text-rose-400">-{showDetail.quantity} {showDetail.products?.unit}</p>
              </div>
              <div className="bg-gray-50 dark:bg-gray-800 rounded-xl p-3">
                <p className="text-xs text-gray-400 mb-1">Harga Saat Itu</p>
                <p className="font-bold text-gray-800 dark:text-gray-100">Rp {Number(showDetail.price_at_time).toLocaleString('id-ID')}</p>
              </div>
              <div className="bg-gray-50 dark:bg-gray-800 rounded-xl p-3 col-span-2">
                <p className="text-xs text-gray-400 mb-1">Total</p>
                <p className="font-bold text-gray-900 dark:text-gray-100 text-lg">Rp {(showDetail.quantity * showDetail.price_at_time).toLocaleString('id-ID')}</p>
              </div>
            </div>
            {[
              { label: 'Alasan', value: <ReasonBadge reason={showDetail.reason} /> },
              { label: 'Keterangan', value: showDetail.reason_note || '-' },
      ...
      {/* Form Drawer */}
      <Drawer show={showForm} onClose={() => { setShowForm(false); resetForm() }} title={editId ? 'Edit Transaksi' : 'Catat Barang Keluar'}>
        <form onSubmit={handleSubmit} onKeyDown={e => { if (e.key === 'Enter') e.preventDefault() }} noValidate className="flex flex-col gap-4">

          {/* Produk — SearchableSelect */}
          <div>
            ...
            {selectedProduct && (
              <div className={`mt-2 p-2.5 rounded-xl text-sm ${selectedProduct.stock <= selectedProduct.min_stock ? 'bg-rose-50 dark:bg-rose-950/40 text-rose-600 dark:text-rose-400' : 'bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-400'}`}>
                Stok tersedia: <span className="font-bold">{editId ? selectedProduct.stock + form.old_quantity : selectedProduct.stock} {selectedProduct.unit}</span>
              </div>
            )}
          </div>
          ...
          {/* Jumlah Keluar */}
          <div>
            <label className="text-xs text-gray-500 mb-1 block">Jumlah Keluar <span className="text-rose-500">*</span></label>
            <div className="flex">
              <input id="field-quantity" type="number" min="1" placeholder="0" value={form.quantity} onChange={e => setField('quantity', e.target.value)}
                className={`border rounded-l-xl p-2.5 text-sm flex-1 min-w-0 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-rose-400 transition-colors ${(fieldErrors.quantity || stockWarning) ? 'border-rose-400 dark:border-rose-500' : 'border-gray-200 dark:border-gray-700'}`} />
            ...
            {stockWarning && !fieldErrors.quantity && (
              <div className="flex items-center gap-1.5 mt-1 text-rose-500 text-xs">
          ...
          {selectedProduct && (
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <div className="border border-gray-200 dark:border-gray-700 rounded-xl p-2.5 text-sm bg-gray-50 dark:bg-gray-800/60 text-gray-600 dark:text-gray-400">
                  Rp {Number(selectedProduct.price).toLocaleString('id-ID')}
                </div>
                <p className="text-xs text-gray-400 mt-1">Otomatis dari produk</p>
              </div>
              <div>
                <label className="text-xs text-gray-500 mb-1 block">Total</label>
                <div className="border border-gray-200 dark:border-gray-700 rounded-xl p-2.5 text-sm bg-gray-50 dark:bg-gray-800/60 font-semibold text-gray-800 dark:text-gray-100">
                  Rp {form.quantity ? (parseInt(form.quantity || 0) * selectedProduct.price).toLocaleString('id-ID') : '0'}
                </div>
              </div>
            </div>
          )}

          {/* Alasan Keluar — CustomSelect */}
          <div>
            <label className="text-xs text-gray-500 mb-1 block">Alasan Keluar <span className="text-rose-500">*</span></label>
            <div id="field-reason" className="w-full">
              <CustomSelect
                value={form.reason}
                onChange={val => setField('reason', val)}
                options={[{ value: '', label: 'Pilih alasan keluar' }, ...reasonFormOptions]}
          ...
          {form.reason === 'Lainnya' && (
            <div>
              <label className="text-xs text-gray-500 mb-1 block">Jelaskan alasan <span className="text-gray-400 font-normal">(opsional)</span></label>
              <textarea placeholder="Jelaskan alasan lainnya..." value={form.reason_note} onChange={e => setField('reason_note', e.target.value)} rows={2} className={inputCls(false) + ' resize-none'} />
            </div>
          )}
          ...
          <div>
            ...
            <textarea placeholder="Catatan tambahan" value={form.note} onChange={e => setField('note', e.target.value)} rows={2} className={inputCls(false) + ' resize-none'} />
          </div>

          {/* Preview stok */}
          {selectedProduct && form.quantity && !stockWarning && (
            <div className="bg-blue-50 dark:bg-blue-950/40 rounded-xl p-3 text-xs text-blue-600 dark:text-blue-400">
              Stok akan diperbarui: <span className="font-semibold">
                {editId ? selectedProduct.stock + form.old_quantity : selectedProduct.stock} → {(editId ? selectedProduct.stock + form.old_quantity : selectedProduct.stock) - parseInt(form.quantity || 0)} {selectedProduct.unit}
              </span>
            </div>
          )}
          ...
          <div className="flex gap-2 pt-2">
            ...
            <button type="submit" disabled={loading || !!stockWarning} className="flex-1 bg-rose-500 text-white rounded-xl p-2.5 text-sm font-semibold hover:bg-rose-600 disabled:opacity-60">
      ...
      <div className="px-4 md:px-6 py-6 max-w-8xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Barang Keluar</h1>
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Catat penjualan dan pengeluaran stok</p>
          </div>
          <div className="flex gap-2">
            ...
            <button onClick={() => { resetForm(); setShowForm(true) }} className="flex items-center gap-2 bg-rose-500 text-white text-sm px-4 py-2.5 rounded-xl hover:bg-rose-600 font-medium">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
          <div className="bg-rose-50 dark:bg-rose-950/40 rounded-2xl p-4 flex items-center gap-4">
            <div className="bg-rose-100 dark:bg-rose-900/60 p-3 rounded-xl shrink-0"><ArrowUpCircle size={20} className="text-rose-500 dark:text-rose-400" /></div>
            <div className="min-w-0">
              <p className="text-xs text-rose-500 dark:text-rose-400 mb-0.5">Total Riwayat</p>
              <p className="text-2xl font-bold text-rose-600 dark:text-rose-300">{stats.total}</p>
              <p className="text-xs text-rose-400 dark:text-rose-500">transaksi</p>
            </div>
          </div>
          <div className="bg-orange-50 dark:bg-orange-950/40 rounded-2xl p-4 flex items-center gap-4">
            <div className="bg-orange-100 dark:bg-orange-900/60 p-3 rounded-xl shrink-0"><Package size={20} className="text-orange-500 dark:text-orange-400" /></div>
            <div className="min-w-0">
              <p className="text-xs text-orange-500 dark:text-orange-400 mb-0.5">Total Item Keluar</p>
              <p className="text-2xl font-bold text-orange-600 dark:text-orange-300">{stats.totalItem}</p>
              <p className="text-xs text-orange-400 dark:text-orange-500">item</p>
            </div>
          </div>
          ...
          <div className="bg-amber-50 dark:bg-amber-950/40 rounded-2xl p-4 flex items-center gap-4">
            <div className="bg-amber-100 dark:bg-amber-900/60 p-3 rounded-xl shrink-0"><TrendingDown size={20} className="text-amber-600 dark:text-amber-400" /></div>
            <div className="min-w-0">
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-4 mb-6">
          <div className="flex flex-col gap-3">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <CustomSelect value={filterReason} onChange={setFilterReason} options={reasonFilterOptions} placeholder="Semua Alasan" />
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 dark:bg-gray-800/60 text-gray-500 dark:text-gray-400 text-xs uppercase tracking-wide">
              <tr>
                ...
                <th className="px-5 py-3 text-left">Harga Saat Itu</th>
            ...
            <tbody>
              ...
              {!fetching && paginated.map(h => (
                <tr key={h.id} className="border-t border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/40 transition-colors">
                  ...
                  <td className="px-5 py-3">
                    <span className="bg-rose-50 dark:bg-rose-950/40 text-rose-500 dark:text-rose-400 font-semibold text-xs px-2.5 py-1 rounded-full">
                      -{h.quantity} {h.products?.unit}
                    </span>
                  </td>
                  <td className="px-5 py-3 text-gray-600 dark:text-gray-300">Rp {Number(h.price_at_time).toLocaleString('id-ID')}</td>
                  <td className="px-5 py-3 font-semibold text-gray-800 dark:text-gray-100">Rp {(h.quantity * h.price_at_time).toLocaleString('id-ID')}</td>
                  <td className="px-5 py-3"><ReasonBadge reason={h.reason} /></td>
                  <td className="px-5 py-3">
              ...
              {!fetching && paginated.length === 0 && (
                <tr>
                  <td colSpan={7} className="px-5 py-16 text-center">
                    <ArrowUpCircle size={40} className="text-gray-200 mx-auto mb-3" />
                    <p className="text-gray-400 text-sm">Belum ada data barang keluar</p>
                    <button onClick={() => { resetForm(); setShowForm(true) }} className="mt-3 text-rose-500 text-sm hover:underline">+ Catat sekarang</button>
                  </td>
                </tr>
              )}
            </tbody>
          ...
          {paginated.map(h => (
            <div key={h.id} className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-4">
              ...
              <div className="flex justify-between text-xs text-gray-400 mb-1">
                ...
                <span className="font-semibold text-gray-700 dark:text-gray-300">Rp {(h.quantity * h.price_at_time).toLocaleString('id-ID')}</span>
              </div>
              ...
              {getPaginationPages().map((n, i) =>
                n === '...'
                  ...
                  : <button key={n} onClick={() => setPage(n)}
                      className={`w-8 h-8 rounded-lg text-sm ${page === n ? 'bg-rose-500 text-white font-medium' : 'hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-600 dark:text-gray-300'}`}>
                      {n}
                    </button>
              )}
        ...
        <button onClick={() => { resetForm(); setShowForm(true) }}
          className="md:hidden fixed bottom-6 right-6 w-14 h-14 bg-rose-500 text-white rounded-full shadow-lg flex items-center justify-center z-40">
          <Plus size={24} />
        </button>
